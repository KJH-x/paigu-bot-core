...
| ID | 状态 | 任务 | 依赖 | 验收 |
|---|---|---|---|---|
| T-01 | todo | **减均重构（A-1）**：`B=Σ各包实付价`、`C=无折扣商品总价`、`D=C−B+G`，按各商品标价加权分摊 `D`；**2 位小数 + 四舍五入 + 残差校正**；校验 `Σ final_i + G = B` | A-1🟡 | 单测覆盖校验式 + 边界 `C=0` |
| T-02 | todo | **特典认购 min(认购数, P)**（A-2）：`G=Σ_X min(claims_X,P)×price_X`；超认购部分掉落 | A-2🟡 | 3A+2B 成 2/3 包两用例 |
| T-03 | todo | **管理员价格表 + 接入结算（A-4）**：可编辑价格表（config）+ `GET/PUT /api/settlement/prices` + `/settlement` 可编辑表格；通行证辅助（25×盒内数、默认 50/仅精一 25） | A-4 | 试算金额非 0 且可复算 |
